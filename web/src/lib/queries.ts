...
export const ALL_CAMPUSES_QUERY = /* groq */ `*[_type == "campus"] | order(name asc){
  ...
  heroImage, bookingIframeUrl, showOnFindCampus, agesServed, location, instagram, facebook, googleBusinessUrl,
...
export const CAMPUS_BY_SLUG_QUERY = /* groq */ `*[_type == "campus" && slug.current == $slug][0]{
  name, "slug": slug.current, region, address, city, postalCode, phone, email, hours, about,
  heroImage, bookingIframeUrl, agesServed, location, instagram, facebook, googleBusinessUrl,
...
export const TESTIMONIALS_QUERY = /* groq */ `*[_type == "testimonial" && ($campus == "" || campus->slug.current == $campus)]
